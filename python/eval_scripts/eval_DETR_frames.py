...
from src.utils import load_DETR_overseer, visualise_results, process_detr_outputs
...
def eval(chckpt: Path,
    ...
    for step, (images, masks, _, _) in enumerate(tqdm(dl, desc=f'Evaluating')):
        ...
        images_list, targets = prepare_data(images, masks, ds, ignore_indices,
                                            device=device, shift_by_1=True,
                                            keep_ignore=keep_ignore,
        ...
        inputs = processor(images=[image.cpu().numpy() for image in images_list],
                           do_rescale=False,  # False // TODO: Rescale to match backbone training image size?
        ...
        #print(f"{targets[0]['masks'].shape=}\t{targets[0]['masks'].min()=}\t{targets[0]['masks'].max()=}")
        #print(f"{processed_outputs[0]['masks'].shape=}\t{processed_outputs[0]['masks'].min()=}\t{processed_outputs[0]['masks'].max()=}")
        #exit()
