...
def train(epochs: int = 100,
          ...
          log_dir: Path = Path('results/mask_rcnn_cholecseg/'),
          device: str = 'cuda'):
    ...
    # ce_weights = compute_class_weights(train_ds, num_classes)
    ce_weights = None

    roi_heads.maskrcnn_loss = lambda a, b, c, d, e: weighted_maskrcnn_loss(a, b, c, d, e, weight=ce_weights)
...
if __name__ == "__main__":
    train(epochs=500,
          ...
          img_norm=(0.0, 1.0),
          ignore_ids=[0],
          shift_ids_by_1=True,  # Must be true since we remove class 0
          components=False,
          ...
          log_dir=Path('results/mask_rcnn___resnet50_64ft_3b'
                       '___cholecseg___step_lr'
                       '___no_comp___ignore_0')
          )
