                ...
                for step, (images, masks, _, _) in enumerate(tqdm(val_dl, desc=f'Validation epoch {epoch}')):
                    ...
                    for i, output in enumerate(outputs):
                        ...
                        if len(pred_boxes) > 0 and len(gt_boxes) > 0:
                            ...
                            all_iou_scores.extend(iou_scores.cpu().numpy())

                            # Compute precision, recall, and F1 score based on IoU threshold
                            true_positives = iou_scores > iou_threshold
                            precision = true_positives.sum() / len(pred_boxes) if len(pred_boxes) > 0 else 0.0
                            recall = true_positives.sum() / len(gt_boxes) if len(gt_boxes) > 0 else 0.0
                    ...
                    """
                            ...
                            # TODO: Include in final evaluation
...
if __name__ == "__main__":
    train(epochs=1000,
          ...
          log_dir=Path('results/mask_rcnn___resnet18_32ft_5b'
                       '___cataracts1k___weighted_sampling___weighted_loss'
