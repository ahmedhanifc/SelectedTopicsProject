...
def train(epochs: int = 100,
          ...
          num_queries: int = 100,
          data_dir: Path = Path('local/scratch/CaDISv2/'),
          log_dir: Path = Path('results/DETR_cadis/'),
          device: str = 'cuda'):
...
if __name__ == "__main__":
    train(epochs=1000,
          ...
          scheduler_gamma=0.5,
          img_size=(200, 200),  # (200x200) // Matching DETR training data (299x299 for Mask R-CNN)
          img_norm=(0.0, 1.0),
          ignore_ids=[],  # ADDITIONAL indices to ignore besides the default ones from the dataset class
          shift_ids_by_1=True,  # Must be true since we remove ignore_index=0  # TODO
          components=False,
          min_comp_fraction=0.0,
          backbone='ResNet50',
          num_queries=30,
          # data_dir=Path('/home/yfrisch_locale/DATA/CaDISv2/'),
          data_dir=Path('/local/scratch/CaDISv2/'),
          log_dir=Path('results/DETR___30queries'
                       '___cadis___200pix___8bs___weighted_sampling'
                       '___step_lr___no_comp___ignore_255/')
          )
