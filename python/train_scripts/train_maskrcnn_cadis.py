...
def train(epochs: int = 100,
          ...
          log_dir: Path = Path('results/mask_rcnn_cadis/'),
          device: str = 'cuda'):
    ...
    for epoch in range(epochs):
        ...
        for step, (images, masks, _, _) in enumerate(tqdm(train_dl, desc=f'Training epoch {epoch}')):
            ...
            images_list, targets = prepare_data(images, masks, train_ds, ignore_ids,
                                                device=device, shift_by_1=shift_ids_by_1, keep_ignore=False,
...
if __name__ == "__main__":
    train(epochs=500,  # 500
          ...
          log_dir=Path('results/mask_rcnn___resnet18_16ft_3b_custom'
                       '___cadis___weighted_sampling___weighted_loss'
                       '___step_lr___no_comp___ignore_1_5_6/')
          )
